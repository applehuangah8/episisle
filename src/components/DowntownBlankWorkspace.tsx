...
const BlankDockedBlock = memo(function BlankDockedBlockInner({
  ...
  worldBoxRef: RefObject<HTMLDivElement | null>;
}) {
  ...
  const handleDragEnd = (_e: PointerEvent | MouseEvent | TouchEvent, info: PanInfo) => {
    ...
    /** 以游標釋放點判定；積木被父層 overflow 裁切時，方塊矩形中心可能仍在右欄內，導致無法回到主畫布 */
    ...
    if (blankVp instanceof HTMLElement) {
      ...
      if (cx >= vr.left && cx <= vr.right && cy >= vr.top && cy <= vr.bottom) {
        ...
        /** 用積木實際矩形中心換算世界座標，避免 Framer drag 與游標點不一致時放開瞬間跳到左上 */
        const br = motionRef.current?.getBoundingClientRect();
        const ax = br ? br.left + br.width / 2 : cx;
        const ay = br ? br.top + br.height / 2 : cy;
        const pt = clientToDowntownBlankWorld(ax, ay, blankVp, pos, sc);
      ...
      >
        ...
        >
          ...
          >
            ...
            {blankIds.map((id) => (
              <BlankDockedBlock key={id} placementId={id} worldBoxRef={worldBoxRef} />
            ))}
          </div>
        </div>
      </div>
